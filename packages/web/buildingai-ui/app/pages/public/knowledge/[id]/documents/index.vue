...
const searchForm = reactive<QueryDocumentParams>({
    keyword: "",
    knowledgeId: kid.value as string,
});
...
type IconType = keyof typeof fileTypeIcons;
...
const columns: TableColumn<DocumentResponse>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const documentType = row.original["documentType"] as IconType;
...
const handleCreate = async () => {
    try {
        ...
        await apiCreateDocument(unref(files), unref(kid) as string, refresh);
...
const handleDelete = async (docId: string) => {
    try {
        ...
        await apiDeleteDocument(docId, kid.value as string);
        await refresh?.();
